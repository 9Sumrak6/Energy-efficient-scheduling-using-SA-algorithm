...
int main() {
	...
	cin >> j;
	cout << "Enter the number of processors (their number should not exceed half of the number of works):\n>";
	cin >> p;
	name = std::to_string(j) + "_" + std::to_string(p) + ".txt";

	cout << endl << "The name of the files will be set in the following format: <number_of_jobs>_<number_of_processors>.txt\nDo you agree with this name? [y/n]: ";
	cin >> move;

	while (move != "y" && move != "n") {
		cout << "Do you agree with this name? [y/n]: ";
		cin >> move;
	}
	cout << endl;
	if (move == "n") {
	...
	cout << endl << name << " files with data, system params and optimum will be created in 'Input/data/', 'Input/sys/', 'Input/opt/' respectively" << endl;
	...
	string path_data = "Input/data/" + name;
	string path_system = "Input/sys/" + name;
	string path_opt = "Input/opt/" + name;
