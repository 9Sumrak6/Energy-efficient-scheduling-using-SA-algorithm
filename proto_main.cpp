...
#include "Utilities/protocol/graph.pb.h"
#include "Utilities/protocol/schedule.pb.h"
    ...
    if (rank == COORDINATOR) {
        ...
        for (auto j : _jobs) {
            for (auto p : _procs) {
                ...
                std::string path_res = "Output/e-8/test.txt"  /* + std::to_string(j) + "_" + std::to_string(p) +  "_e-8.txt"*/;
